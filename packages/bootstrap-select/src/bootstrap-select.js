 ...
 * Shoutout to Angular 7 https://github.com/angular/angular/blob/7.2.4/packages/core/src/sanitization/url_sanitizer.ts
 */
let DATA_URL_PATTERN =
  /^data:(?:image\/(?:bmp|gif|jpeg|jpg|png|tiff|webp)|video\/(?:mpeg|mp4|ogg|webm)|audio\/(?:mp3|oga|ogg|opus));base64,[a-z0-9+/]+=*$/i;

let ParseableAttributes = ["title", "placeholder"]; // attributes to use as settings, can add others in the future
...
function allowedAttribute(attr, allowedAttributeList) {
  ...
  let regExp = $(allowedAttributeList).filter(function (index, value) {
...
let Dropdown = window.Dropdown || bootstrap.Dropdown;
...
let REGEXP_TAB_OR_ESCAPE = new RegExp(
  "^" + keyCodes.TAB + "$|" + keyCodes.ESCAPE
);

let generateOption = {
  li(content, classes, optgroup) {
    let li = elementTemplates.li.cloneNode(false);
  ...
  a(text, classes, inline) {
    let a = elementTemplates.a.cloneNode(true);
...
class Selectpicker {
  ...
  init() {
    ...
    this.$button = this.$newElement.childNodes.item("button");
    if (this.options.allowClear)
      this.$clearButton = this.$button.childNodes.item(
        ".bs-select-clear-selected"
      );
    this.$menu = this.$newElement.childNodes.item(Selector.MENU);
    this.$menuInner = this.$menu.childNodes.item(".inner");
    ...
    this.$menu.dataset.this = this;
    this.$newElement.dataset.this = this;
    if (this.options.mobile) this.mobile();

    const events = ['hide', 'hidden', 'show', 'shown'];

    events.forEach(event => {
      this.$newElement.addEventListener(event + EVENT_KEY, function(e) {
        ...
        that.$element
          ...
          .on("rendered" + EVENT_KEY, function () {
            ...
            if (this.validity.valid)
              that.$button.classList.remove("bs-invalid");
    ...
    function scroll(scrollTop, init) {
      ...
      if (init || positionIsDifferent || that.selectpicker.current.hasMore) {
        ...
        if ((init || isVirtual === true) && menuIsDifferent) {
          ...
          for (
            let i = 0, visibleElementsLen = elements.length;
  ...
  buildData(data, type) {
    let that = this;
    let dataGetter =
      data === false ? getOptionData.fromOption : getOptionData.fromDataSource;
    ...
    let selectOptions = data
      ...
      : this.$element.querySelectorAll("select > *" + optionSelector);
  ...
  render(init) {
    ...
    if (this.options.selectedTextFormat === "static") {
      ...
      if (showCount === false) {
        ...
        let totalCount = this.$element.querySelectorAll(
            "select > option" +
              optionSelector +
              ", optgroup" +
              optionSelector +
    ...
    if (
      version.major < 4 &&
      this.$newElement.classList.contains("bs3-has-addon")
    ) {
  ...
  liHeight(refresh) {
    ...
    let newElement = elementTemplates.div.cloneNode(false),
      ...
      header =
        this.options.header &&
        this.$menu.querySelector("." + classNames.POPOVERHEADER)
      ...
      actions =
        ...
        this.multiple &&
        this.$menu.querySelector(".bs-actionsbox")
          ? this.$menu.querySelector(".bs-actionsbox").cloneNode(true)
          : null,
      doneButton =
        this.options.doneButton &&
        this.multiple &&
        this.$menu.querySelector(".bs-donebutton")
    ...
    newElement.className =
      this.$menu.parentNode.className + " " + classNames.SHOW;
  ...
  setWidth() {
    ...
    if (this.options.width === "auto") {
      requestAnimationFrame(function () {
        ...
        that.$element.on("loaded" + EVENT_KEY, function () {
          ...
          let $selectClone = that.$newElement.clone().appendTo("body"),
    ...
    if (
      this.$newElement.classList.contains("fit-width") &&
  ...
  clickListener() {
      ...
      $item.addEventListener("click", function (e, retainActive) {
        ...
        if (
          ...
          let option = clickedData.option,
            ...
            optgroupData = that.selectpicker.current.data.find(function (
              datum
            ) {
              return (
    ...
    const $targets = this.$menuInner.querySelectorAll(
    ...
    const $popoverClose = this.$menu.querySelector(
  ...
  val(value) {
    ...
    if (typeof value !== "undefined") {
      let selectedOptions = getSelectedOptions.call(this),
      ...
      if (this.$newElement.hasClass(classNames.SHOW)) {
        if (this.multiple) {
          ...
          let liSelectedIndex = (element.options[element.selectedIndex] || {})
            .liIndex;
  ...
  changeAll(status) {
    ...
    for (
      let i = 0, data = this.selectpicker.current.data, len = data.length;
      i < len;
      i++
    ) {
      let liData = data[i],
